...
/* ----------------------------------------------------------------------
   ...
   Theme derived from the Forfra brand: deep navy (#0B1F4D / #0E2A5E),
   warning-gold accent (#D8A93B), forensic-blueprint texture motifs,
   set in a disciplined "case file" layout — numbered exhibits,
   ledger-style data rows, evidence-tag corners.
------------------------------------------------------------------------- */
...
const PROCESS_STEPS = [
  {
    ...
    text: "We define the financial scope, regulatory triggers, and data sources relevant to the suspected irregularity before any record is touched.",
    icon: FileSearch,
  },
  {
    n: "02",
    title: "Evidence & Data Capture",
    text: "Financial records and digital ledgers are imaged and preserved under a verifiable chain of custody, protecting admissibility from day one.",
    icon: ShieldCheck,
  },
  {
    n: "03",
    title: "Investigative Analytics",
    text: "Certified examiners apply forensic analytics to trace discrepancies, irregular transactions, and patterns indicating fraud or misconduct.",
    icon: Search,
  },
  {
    n: "04",
    title: "Verification & Cross-Check",
    text: "Findings are cross-verified against statutory and compliance benchmarks to confirm lawful conduct or substantiate irregularities.",
    icon: TrendingUp,
  },
  {
    n: "05",
    title: "Court-Admissible Reporting",
    text: "A clear, defensible report is prepared — suitable for regulators, law enforcement, and litigation — so your organization stays protected.",
    icon: Scale,
  },
];

const INDUSTRIES = [
  { label: "Banking & Financial Services (NBFCs, Fintech)", icon: Landmark },
  { label: "Manufacturing & Trading Companies", icon: Factory },
  { label: "Real Estate & Infrastructure", icon: Building2 },
  { label: "E-commerce & Startups", icon: ShoppingBag },
  { label: "Healthcare & Hospitals", icon: HeartPulse },
  { label: "Government & PSU Projects", icon: Landmark },
  { label: "Education Institutions (Universities & Colleges)", icon: GraduationCap },
  { label: "Insurance Companies", icon: ShieldCheck },
  { label: "Retail Chains & FMCG", icon: Boxes },
  { label: "NGOs & Trusts", icon: HandHeart },
];

const WHY_POINTS = [
  "Certified forensic examiners with investigative + accounting expertise",
  "Court-admissible reports built for regulatory and legal scrutiny",
  "Full chain-of-custody discipline on every financial record handled",
  "ISO 27001:2022 and ISO 9001:2015 certified operating standards",
];
    ...
    <div className="fa-root">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .fa-ind-grid {
          position:relative; z-index:1; display:grid; grid-template-columns: repeat(5, 1fr); gap:14px; margin-top:44px;
        }
        @media (max-width:980px){ .fa-ind-grid{ grid-template-columns: repeat(3,1fr); } }
        @media (max-width:620px){ .fa-ind-grid{ grid-template-columns: repeat(2,1fr); } }
        .fa-ind-card {
        ...
        .fa-ind-card svg { color: var(--gold); margin-bottom:12px; }
        .fa-ind-card span { color: rgba(255,255,255,0.85); font-size:0.85rem; font-weight:500; line-height:1.4; display:block; }
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .fa-marquee-track, .fa-spin-slow, .fa-glow-card::before { animation: none !important; }
        }
      `}</style>
      ...
      <header className="fa-hero">
        ...
        <div className="fa-eyebrow">
          <ShieldCheck size={14} className="fa-spin-slow" /> ISO 27001:2022 &amp; ISO 9001:2015 Certified
        </div>
        <h1>
          Forensic Audit —<br />
          uncovering <em>fraud</em>, before it<br />
          uncovers you.
        </h1>
        <p className="lede">
          Our forensic auditing services go beyond traditional audits — identifying fraud,
          financial irregularities, and hidden risks with precision and integrity. During
          regulatory scrutiny, law enforcement searches, or compliance investigations, our
          certified experts analyze financial and digital records to verify lawful conduct
          and deliver court-admissible reports.
        </p>
        <div className="fa-hero-actions">
          <button className="fa-btn-primary">
            Start an Audit Engagement <ArrowRight size={17} />
          </button>
          <button className="fa-btn-ghost">View Our Methodology</button>
        </div>
        <div className="fa-hero-stats">
          <StatCounter value={10} label="Industries Served" />
          <StatCounter value={6} label="Regulatory Bodies Engaged" />
          <StatCounter value={100} suffix="%" label="Court-Admissible Reporting" />
        </div>
      </header>
      ...
      <div className="fa-marquee-wrap">
        <div className="fa-marquee-track">
          {Array.from({ length: 2 }).flatMap((_, dup) =>
            [
              ["ISO 27001:2022 Information Security", ShieldCheck],
              ["ISO 9001:2015 Quality Management", CheckCircle2],
              ["Court-Admissible Reporting", Scale],
              ["Chain of Custody Compliance", FileSearch],
              ["DPDP Act 2023 Aligned", ShieldCheck],
              ["Section 63(4)(c) Certified Process", Scale],
            ].map(([label, Icon], i) => (
              <span key={`${dup}-${i}`}>
                <Icon size={15} /> {label}
              </span>
      ...
      <section className="fa-section" id="about">
        <Reveal>
          ...
          <h2 className="fa-h2">Investigative expertise, combined with advanced analytics</h2>
        </Reveal>
        <div className="fa-intro-wrap" style={{ marginTop: 40 }}>
          <Reveal delay={80}>
            <p className="fa-sub" style={{ fontSize: "1.05rem" }}>
              Forfra Solutions is a multidisciplinary professional firm operating at the
              intersection of technology, investigation, and intelligence — specializing in
              Data Security, Forensic Audit, Digital Forensics, Crime Investigation, and
              Financial Fraud Analysis for corporates, law enforcement agencies, and academic
              institutions.
            </p>
            <p className="fa-sub" style={{ marginTop: 18 }}>
              We combine investigative expertise with advanced analytics to trace
              discrepancies, uncover fraud, and support legal proceedings — delivering
              fact-based insights you can trust, from fraud detection through to litigation
              support.
            </p>
          </Reveal>
          <Reveal delay={160}>
            <div className="fa-intro-card fa-glow-card">
              <h3>Built for Regulatory Scrutiny</h3>
              <p>
                When law enforcement searches, regulatory reviews, or compliance investigations
                arise, our certified examiners analyze financial and digital records to verify
                lawful conduct and produce reports that hold up where it matters most — in
                court.
              </p>
              <div className="fa-intro-quote">
                <Quote size={20} />
                <span>"Fact-based insights you can trust — from fraud detection to litigation support."</span>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      {/* PROCESS */}
      <section className="fa-section tight" id="process" style={{ background: "#fff" }}>
        <Reveal>
          <div className="fa-kicker">Our Methodology</div>
          <h2 className="fa-h2">A defensible audit trail, from engagement to courtroom</h2>
          <p className="fa-sub">
            Every Forfra forensic audit follows a disciplined, evidence-first sequence —
            engineered so findings remain accurate, traceable, and legally admissible at every
            stage.
          </p>
        </Reveal>
      ...
      <section className="fa-section fa-why" id="why">
        <Reveal>
          <div className="fa-kicker">Why Organizations Choose Forfra</div>
          <h2 className="fa-h2">Security with accountability, not just audit checkboxes</h2>
          <p className="fa-sub">
            We combine security with accountability — we don't just review numbers, we build
            chain-of-custody discipline and legal compliance into every engagement.
          </p>
        </Reveal>
      ...
      {/* INDUSTRIES */}
      <section className="fa-section fa-industries-band" id="industries">
        <div className="fa-ind-head">
          <Reveal>
            <div className="fa-kicker">Targeted Industries</div>
            <h2 className="fa-h2">Forensic audit experience across every regulated sector</h2>
            <p className="fa-sub">
              From banking and fintech to healthcare, education, and government PSU projects —
              our forensic auditors bring sector-specific fluency to every engagement.
            </p>
          </Reveal>
        </div>
      ...
      <Reveal>
        <section className="fa-cta" id="contact">
          ...
          <div>
            <h3>Whether it's fraud detection or litigation support, we're ready.</h3>
            <p>
              Our certified forensic audit team delivers accurate, court-admissible findings
              when it matters most. Let's protect what matters to your organization.
            </p>
          </div>
